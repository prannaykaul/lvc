...
# ROI_HEADS_OUTPUT_REGISTRY = Registry("ROI_HEADS_OUTPUT2")
# ROI_HEADS_OUTPUT_REGISTRY.__doc__ = """
# Registry for the output layers in ROI heads in a generalized R-CNN model, with second highest also return"""
...
def fast_rcnn_inference_debug(
    ...
    """
    result_per_image = [
        fast_rcnn_inference_single_image_debug(
            boxes_per_image,
            scores_per_image,
            image_shape,
            score_thresh,
...
def fast_rcnn_inference_single_image_debug(
    ...
    # top2_scores, top2_inds = scores.topk(2, dim=-1)
    ...
    # top2_scores = top2_scores[filter_inds[:, 0]]
    # top2_inds = top2_inds[filter_inds[:, 0]]
    # Apply per-class NMS
    # keep = batched_nms(boxes, scores, filter_inds[:, 1], nms_thresh)
    # if topk_per_image >= 0:
    #     keep = keep[:topk_per_image]
    # boxes, scores, filter_inds = boxes[keep], scores[keep], filter_inds[keep]
    # top2_scores, top2_inds = top2_scores[keep], top2_inds[keep]
    # print(boxes.size(), scores.size(), scores.max(), scores.min())
    result = Instances(image_shape)
    result.pred_boxes = Boxes(boxes)
    result.scores = scores
    result.bkg_scores = bkg_scores.repeat_interleave(scores.numel()//bkg_scores.numel())
    result.pred_classes = filter_inds[:, 1]
    # result.top2_scores = top2_scores
    # result.top2_inds = top2_inds
    return result, filter_inds[:, 0]


class FastRCNNOutputsDebug(object):
    """
    ...
    def __init__(
        ...
        if proposals[0].has("gt_boxes"):
            ...
            if proposals[0].has("gt_og_classes"):
                self.gt_og_classes = cat([p.gt_og_classes for p in proposals], dim=0)
                self.ig = True
            else:
                self.ig = False
    ...
    def predict_probs(self):
        ...
        return self.pred_class_logits.split(self.num_preds_per_image, dim=0)
    ...
    def inference(self, score_thresh, nms_thresh, topk_per_image):
        ...
        return fast_rcnn_inference_debug(
            boxes,
            scores,
            image_shapes,
            score_thresh,
