...
def parse_args():
    ...
    parser.add_argument('--save-dir', type=str, default='',
                        help='Save directory')
    # Surgery method
    parser.add_argument('--method', choices=['combine', 'remove', 'randinit', 'combine_parallel'],
                        required=True,
...
def combine_parallel_ckpts(args):
    ckpt_base = torch.load(args.src1, map_location=torch.device('cpu'))
    ckpt_novel = torch.load(args.src2, map_location=torch.device('cpu'))
    save_name = args.tar_name + '_combine_parallel.pth'
    if args.save_dir == '':
    ...
    reset_ckpt(ckpt_base)
    new_ckpt = copy.deepcopy({k: v for k, v in ckpt_base['model'].items() if 'roi_heads' not in k})
    for k, v in ckpt_base['model'].items():
        if 'roi_heads' in k:
            new_ckpt[k.replace('roi_heads', 'roi_heads.roi_heads.base')] = copy.deepcopy(v)
    for k, v in ckpt_novel['model'].items():
        if 'roi_heads' in k:
            new_ckpt[k.replace('roi_heads', 'roi_heads.roi_heads.novel')] = copy.deepcopy(v)
    save_ckpt({'model': new_ckpt}, save_path)
    return
...
def surgery_loop(args, surgery):
    ...
    elif args.method == 'combine_parallel':
        ckpt2 = torch.load(args.src2, map_location=torch.device('cpu'))
        save_name = args.tar_name + '_combine_parallel.pth'
    else:
        ckpt2 = None
    ...
    if args.cascade:
        tar_sizes = [TAR_SIZE + 1] * len(args.param_name)
