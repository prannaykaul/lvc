...
class BottleneckBlockDropout(CNNBlockBase):
    def __init__(
        self,
        in_channels,
        out_channels,
...
class BottleneckBlockCLIP(CNNBlockBase):
    """
    The standard bottleneck residual block used by ResNet-50, 101 and 152
    defined in :paper:`ResNet`.  It contains 3 conv layers with kernels
    1x1, 3x3, 1x1, and a projection shortcut if needed.
    ...
    def __init__(
        ...
        *,
        ...
        if in_channels != out_channels or stride > 1:
            self.avgpool_sc = nn.AvgPool2d(stride)
            self.shortcut = Conv2d(
                in_channels,
                out_channels,
                kernel_size=1,
...
def build_resnet_backbone(cfg, input_shape):
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        if depth in [18, 34]:
            ...
            if cfg.MODEL.RESNETS.D:
                stage_kargs["block_class"] = BottleneckBlockCLIP
            elif cfg.MODEL.RESNETS.DROPOUT and stage_idx > freeze_at:
                stage_kargs["block_class"] = BottleneckBlockDropout
                stage_kargs["dropout"] = cfg.MODEL.RESNETS.DROPOUT
