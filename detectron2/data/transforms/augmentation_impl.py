...
class ColorJitterPIL(Augmentation):
    """
    Randomly transforms hue of an RGB image.

    Hue intensity is uniformly sampled in (intensity_min, intensity_max).
    - intensity < 1 will reduce hue
    - intensity = 1 will preserve the input image
    - intensity > 1 will increase hue
    ...
    def get_transform(self, img):
        return PILColorTransform(self.aug)


class GaussianBlurPIL(Augmentation):
    """
    Randomly transforms hue of an RGB image.
    ...
    def __init__(self, kernel_size=(3, 3), sigma=(0.1, 3.0)):
        """
        Args:
            intensity_min (float): Minimum augmentation (1 preserves input).
            intensity_max (float): Maximum augmentation (1 preserves input).
