...
def build_augmentation(cfg, is_train):
    ...
    augmentation = []
    if cfg.INPUT.COLOR_JITTER and is_train:
        augmentation.append(
            T.ColorJitterPIL()
        )
    if cfg.INPUT.BLUR and is_train:
        augmentation.append(
            T.GaussianBlurPIL()
        )
    if is_train and cfg.INPUT.LSJ:
        augmentation.extend([
            T.ResizeScale(min_scale=0.5, max_scale=1.6, target_height=800, target_width=800),
            T.FixedSizeCrop(crop_size=(800, 800))
        ])
    else:
