...
from .fast_rcnn import FastRCNNOutputLayers, fast_rcnn_inference, ROI_HEADS_OUTPUT_REGISTRY
...
class CascadeROIHeads(StandardROIHeads):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.reg_only = kwargs['output_layer'] == "BoxOnlyLayersCascade"
        kwargs.pop('output_layer')
        super().__init__(
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        for match_iou, bbox_reg_weights in zip(cascade_ious, cascade_bbox_reg_weights):
            ...
            box_predictors.append(
                ROI_HEADS_OUTPUT_REGISTRY.get(output_layer)(
                    cfg,
    ...
    def forward(self, images, features, proposals, targets=None):
        del images
        meta_arch = global_cfg.MODEL.META_ARCHITECTURE
        if (self.training or self.reg_only) and meta_arch != "GeneralizedRCNNRegOnly":
            proposals = self.label_and_sample_proposals(
                proposals, targets, inference=(not self.training))

        if meta_arch == "GeneralizedRCNNRegOnly" and not self.training:
            return self._forward_box_qe(features, None, targets)
        ...
        if self.training:
            ...
            if self.reg_only:
                pred_instances, proposals = self._forward_box(features, proposals)
                return pred_instances, proposals
            pred_instances = self._forward_box(features, proposals)
            pred_instances = self.forward_with_given_boxes(features, pred_instances)
            return pred_instances, {}

    def _forward_box_qe(self, features, proposals, targets):
        for x in targets:
        ...
        image_sizes = [x.image_size for x in targets]
        ...
        num_classes = predictions[0]
        boxes = predictor.predict_boxes(predictions, proposals)
        proposals = head_outputs[0][-1]
        scores = list(b.new_zeros(len(p), num_classes+1) for b, p in zip(boxes, proposals))
        ...
        return tuple(o[0][o[1].argsort()] for o in zip(*out)), None
